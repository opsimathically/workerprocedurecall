...
const SHA1_LOOP_ITERATIONS_PER_CALL = 2_000;
...
function RunLocalBenchmark(params: run_local_benchmark_params_t): benchmark_result_t {
  ...
  return BuildBenchmarkResult({
    benchmark_name: 'Single-thread In-process SHA-1 Function',
    total_operations,
    start_time_ms,
    end_time_ms
  });
  ...
  try {
    ...
    await RunParallelOperations({
      ...
      operation_func: async function (operation_index: number): Promise<void> {
        const return_value = await workerprocedurecall.call.WPCBenchmarkSha1();
        ValidateDigestResult({
          value: return_value,
          expected_digest,
          label: 'Worker warmup',
    ...
    return BuildBenchmarkResult({
      benchmark_name: 'WorkerProcedureCall SHA-1 RPC',
      total_operations,
      start_time_ms,
      end_time_ms
    });
...
async function RunBenchmarkSuite(): Promise<void> {
  ...
  console.log('Worker Vs Single-thread SHA-1 Benchmark');
  console.log(
