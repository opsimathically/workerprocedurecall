...
const BENCHMARK_TOTAL_OPERATIONS = 200_000;
const BENCHMARK_WARMUP_OPERATIONS = 10_000;
...
function ValidateOkResult(params: {
  value: unknown;
  label: string;
  operation_index: number;
}): void {
  const { value, label, operation_index } = params;
  if (value !== 'OK!') {
    throw new Error(
      `${label} returned unexpected value at operation ${operation_index}: ${String(value)}`
    );
  }
}
...
function RunLocalBenchmark(params: run_local_benchmark_params_t): benchmark_result_t {
  ...
  for (
    ...
    const return_value = LocalOkFunction();
    ValidateOkResult({
      value: return_value,
      label: 'Local warmup',
      operation_index
  ...
  return BuildBenchmarkResult({
    benchmark_name: 'Single-thread In-process Function',
    total_operations,
    start_time_ms,
    end_time_ms
  });
  ...
  try {
    ...
    await RunParallelOperations({
      ...
      operation_func: async function (operation_index: number): Promise<void> {
        const return_value = await workerprocedurecall.call.WPCBenchmarkOk();
        ValidateOkResult({
          value: return_value,
          label: 'Worker warmup',
          operation_index
    ...
    return BuildBenchmarkResult({
      benchmark_name: 'WorkerProcedureCall RPC',
      total_operations,
      start_time_ms,
      end_time_ms
    });
...
async function RunBenchmarkSuite(): Promise<void> {
  ...
  console.log('Worker Vs Single-thread Benchmark');
  console.log(
